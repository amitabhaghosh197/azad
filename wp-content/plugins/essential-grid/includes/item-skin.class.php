	...
	public function insert_layer($layer, $demo = false, $masonry = false){
		...
		switch($link_to){
			...
			case 'lightbox':
				...
				$text = '<a href="'.$lb_source.'"'.$lb_addition.' lgtitle="'.$lb_title.'">'.$text.'</a>';
		...
		if($base->text_has_certain_tag($text, 'a')){ //check if a tag exists, if yes, class will be set to a tags and not the wrapping div, also the div will receive the position and other stylings // && @$layer['settings']['source'] !== 'text'
			if($is_woo_cats && strpos($text, 'class="') !== false || $is_woo_button || $is_html_source && strpos($text, 'class="') !== false || $is_filter_cat && strpos($text, 'class="') !== false) //add to the classes instead of creating own class attribute if it is woocommerce cats AND a class can be found
